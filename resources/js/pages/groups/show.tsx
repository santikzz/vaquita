...
export default function GroupShow({ group, members, events, balances }: Props) {
  ...
  return (
        ...
        <div className="flex-none px-4 pt-4 pb-1.5">
          <div className="mb-3.5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Link
                href={`/groups/${group.uuid}/stats`}
                prefetch="mount"
                aria-label={t("groups:statistics")}
          ...
          <div className="px-1 pb-1">
            ...
            <div className="mt-0.5 text-[13px] text-muted-foreground">
              {t("groups:members_count", { count: activeMembers.length })} ·{" "}
              {t("groups:events_count", { count: events.length })}
            </div>
          </div>
          <Segmented
            className="mt-3.5"
            ...
            options={[
              {
                value: "events",
                label: t("groups:tab_events"),
                icon: CalendarDays,
              },
              { value: "members", label: t("groups:tab_members"), icon: Users },
              {
                value: "balances",
                label: t("groups:tab_balances"),
                icon: Scale,
              },
            ]}
          />
    ...
    >
      ...
      {tab === "events" && (
        <Fab
          href={`/groups/${group.uuid}/events/create`}
          label={t("groups:event")}
          aboveTabBar
        />
      )}
    </AppShell>
        ...
        >
          <div>
            ...
            <div className="mt-0.5 text-[12.5px] text-muted-foreground">
              {event.event_date} ·{" "}
              {formatMoney(event.total_minor, group.currency)}
            </div>
          </div>
          <span
...
function MembersTab({
  ...
  const copyAlias = async (member: MemberData) => {
    ...
    setTimeout(
      () => setCopiedAlias((prev) => (prev === member.uuid ? null : prev)),
      1600,
    );
  };
        ...
        {members.map((member) => (
          ...
          >
            <MemberAvatar
              ...
              className={member.is_active ? "" : "opacity-40"}
            />
            <div className="min-w-0 flex-1">
              <div
                className={`truncate text-[15px] font-medium ${member.is_active ? "" : "text-muted-foreground line-through"}`}
              >
                {member.display_name}
              </div>
            ...
            {group.is_owner && member.role !== "owner" && member.is_active && (
              <ConfirmDrawer
                onConfirm={() => {
                  ...
                  setMembers((prev) =>
                    prev.map((m) =>
                      m.uuid === member.uuid ? { ...m, is_active: false } : m,
                    ),
                  );
                  router.delete(
                    `/groups/${group.uuid}/members/${member.uuid}`,
                    {
                      preserveScroll: true,
                      only: ["members", "balances"],
                      onError: () =>
                ...
                description={t("groups:remove_member_hint", {
                  name: member.display_name,
                })}
              >
                <button
          ...
          <Drawer open={guestOpen} onOpenChange={setGuestOpen}>
            <DrawerContent>
              <div className="p-4 pb-8">
                <DrawerTitle className="px-1 pb-1.5 text-[15px] font-semibold">
                  {t("groups:add_guest")}
                </DrawerTitle>
                <p className="px-1 pb-3.5 text-[13px] leading-relaxed text-muted-foreground">
                  {t("groups:add_guest_hint")}
                </p>
                <input
                  value={guestName}
...
function BalancesTab({
  ...
  const byMember = new Map(
    balances.map((b) => [b.member_uuid, b.amount_minor]),
  );
  const rows = members
